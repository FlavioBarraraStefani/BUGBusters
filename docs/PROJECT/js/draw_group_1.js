...
function draw_group_1(data, choice, containerId) {
  ...
  // Update margins to ensure axis labels fit, based on CHART_MARGIN
  ...
  function mousemove(event) {
    ...
    // Calculate yearly attacks (Current Cumulative - Previous Cumulative)
    const prevVal = (i > 0 && groupData[i-1]) ? groupData[i-1].value : 0;
    // Note: if d is d0, we need to look at d0's previous index. 
    // Simplified logic: find index of d in data to be sure
